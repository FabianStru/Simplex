...
public class MatrixTest {
    ...
    public void Test_Matrix_RottisBeispiel() {
        ...
        BigFraction[][] bcompare = doubleToBigFractionArray(compare);
    ...
    public BigFraction[][] doubleToBigFractionArray(double[][] doubleArray){
        BigFraction[][] convertedBigFractionArray = new BigFraction[doubleArray.length][doubleArray[0].length];

        for (int i = 0; i < doubleArray.length; i++) {
            for (int j = 0; j < doubleArray[i].length; j++) {
                convertedBigFractionArray[i][j] = new BigFraction(doubleArray[i][j]);
            }
        }

        return convertedBigFractionArray;
    }
